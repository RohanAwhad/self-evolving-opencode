...
Marked @pytest.mark.live — run with:
    uv run pytest tests/test_skill_evolution.py -m live --override-ini="addopts="

All use pre-seeded Redis cache. No real LLM calls needed for cache-hit tests.
"""
...
class TestEvolveBootstrap:
    async def test_insight_only_no_rules(self, redis_client):
        ...
        response = {"insights_by_skill": {"test-skill": ["Always check assertion output first"]}}
...
class TestEvolveExisting:
    async def test_tag_and_curate(self, redis_client):
        ...
        key = cache_key("complete_tool", messages=[{"role": "user", "content": reflect_prompt}], tool=REFLECT_TAG_TOOL, model=DEFAULT_MODEL, max_tokens=4096, system=REFLECT_TAG_SYSTEM)
        await cache_set(r, key, json.dumps(reflect_response, sort_keys=True))
        seeded_keys.append(key)
        ...
        curator_response = {
            "rules": [
                {"content": "Verify the fix in CI before closing the issue", "reasoning": "Agent pushed without CI check"},
            ]
        }
        key2 = cache_key("complete_tool", messages=[{"role": "user", "content": curator_prompt}], tool=CURATOR_TOOL, model=DEFAULT_MODEL, max_tokens=4096, system=CURATOR_SYSTEM)
        await cache_set(r, key2, json.dumps(curator_response, sort_keys=True))
